...
@Component({
  ...
  providers: [DatePipe],
  templateUrl: './audit-refferal.component.html',
  styleUrl: './audit-refferal.component.css'
})
export class AuditRefferalComponent implements OnInit {
  auditLogs: any[] = [];
  isLoading = false;
  currentEntity = 'referral';
  ...
  private parseDiffs(diffsString: string): any {
    try {
      ...
      if (diffs['toDoctor'] && diffs['toDoctor'].new) {
        result['toDoctor'] = { new: diffs['toDoctor'].new.label.split('#')[1] };
      }
      if (diffs['fromDoctor'] && diffs['fromDoctor'].new) {
        result['fromDoctor'] = { new: diffs['fromDoctor'].new.label.split('#')[1] };
      }

      if (diffs['patient'] && diffs['patient'].new) {
        result['patient'] = { new: diffs['patient'].new.label.split('#')[1] };
      }
